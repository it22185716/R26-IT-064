...
function daysLeft(plan: MealPlan): number {
  ...
  // NOTE(review): "days left" appears to render NaN in some cases — worth checking separately.
...
export default function MealPlanPage() {
  ...
  // Print-only summary block (allergies, full meal calendar table, diagnosis
  // row) captured as the PDF's leading page — see meal-plan-print-view below.
  ...
  async function handleDownloadPdf() {
    ...
    try {
      ...
      // +1 for the leading summary page.
      ...
      // Same header (name, subtitle, divider) drawn at the top of every page —
      // the summary page and each day page. Returns the y position where the
      // page's image content should start.
      ...
      const drawPageFooter = (pageNum: number) => {
        ...
        pdf.text(
          `Generated by Hayagiri AI Learning Platform · ${generatedOn} · Page ${pageNum} of ${totalPages}`,
          pageWidth / 2,
          pageHeight - 8,
          { align: 'center' },
        );
      ...
      // cuts a 14-page PDF from ~26MB down to a few MB. Shared by every page
      // so the summary page and day pages never drift apart on this math.
      const addFittedImage = async (el: HTMLElement, y: number) => {
        ...
        // footer) so content is never cut off mid-card.
      ...
      <div className="meal-plan-print-view" aria-hidden="true">
        ...
        <div ref={summaryPrintRef} className="print-summary">
          <section className="print-summary-section">
            <h2 className="print-summary-title">Allergies</h2>
            {currentPlan.profile.allergies.length > 0 ? (
              <div className="print-allergy-tags">
                {currentPlan.profile.allergies.map((a) => (
                  <span key={a} className="print-allergy-tag">
                    {a}
                  </span>
                ))}
              </div>
            ) : (
              <p className="print-summary-empty">No known allergies</p>
            )}
          </section>

          <section className="print-summary-section">
            <h2 className="print-summary-title">Current meal calendar</h2>
          ...
          <section className="print-summary-section print-diagnosis-row">
            <div className="print-diagnosis-item">
              <p className="print-diagnosis-label">Nutritional status</p>
              <p className="print-diagnosis-value">{currentPlan.nutritionalStatus}</p>
            </div>
            <div className="print-diagnosis-item">
              <p className="print-diagnosis-label">BMI</p>
              <p className="print-diagnosis-value">{currentPlan.profile.bmi}</p>
            </div>
            <div className="print-diagnosis-item">
              <p className="print-diagnosis-label">Meal goal</p>
              <p className="print-diagnosis-value">{currentPlan.mealGoal}</p>
            </div>
            <div className="print-diagnosis-item">
              <p className="print-diagnosis-label">Plan dates</p>
              <p className="print-diagnosis-value">
                {formatDate(currentPlan.createdAt)} – {formatDate(currentPlan.expiresAt)}
              </p>
            </div>
            <div className="print-diagnosis-item">
              <p className="print-diagnosis-label">Days left</p>
              <p className="print-diagnosis-value">{isExpired ? 'Expired' : `${remaining}d`}</p>
            </div>
          </section>
        </div>
